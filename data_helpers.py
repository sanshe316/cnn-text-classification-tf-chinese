...
def load_data_and_labels(filename):
    ...
    with open(filename, "rt") as f:
        for line in f:
            ...
            label = []
            for field in fields:
                if field[:9] == "__label__":
                    label.append(field)
                else:
                    sentence.append(field)
            x_text.append(sentence)
            labels.append(label)
    a = set([l for l in label for label in labels])
    labels_all = sorted(list(set([l for label in labels for l in label])))
    y = np.zeros((len(labels),len(labels_all)),dtype=int)
    for i,label in enumerate(labels):
        for l in label:
            if l in labels_all:
                y[i,labels_all.index(l)] = 1
    return [x_text, y]
